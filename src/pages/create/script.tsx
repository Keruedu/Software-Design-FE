...
export default function ScriptPage() {
  ...
  }, [state.selectedTopic, state.keyword, router]);  // Initialize script from state if available
  useEffect(() => {
    if (!hasInitialized) {
      if (state.script) {
        ...
        // Auto-generate script if we don't have one yet and we have a topic
        const topic = state.selectedTopic?.title || state.keyword;
        if (topic && !isGenerating && !hasAutoGenerated) {
          console.log('🚀 Auto-generating script for topic:', topic);
  ...
  }, [state.script, hasInitialized, state.selectedTopic, state.keyword, hasAutoGenerated]);
