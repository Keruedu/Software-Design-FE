...
interface ScriptGeneratorProps {
  ...
  autoGenerate?: boolean; // Add flag to control auto-generation
}
...
const ScriptGenerator: React.FC<ScriptGeneratorProps> = ({
  ...
  useEffect(() => {
    // Only auto-generate if explicitly requested and not already done
    if (autoGenerate && !initialScript && topic && !script && !isGenerating && !hasAutoGenerated) {
      setHasAutoGenerated(true);
      generateScript();
    } else if (initialScript) {
      setScript(initialScript);
    }
  }, [topic, initialScript, autoGenerate, hasAutoGenerated]);
    ...
    try {
      // Use the correct signature: generateScript(topic: string, keywords: string[])
      const generatedScript = await ScriptService.generateScript(topic, keywords);
  ...
  const handleEdit = () => {
    ...
    // Use the correct signature: updateScript(id: string, updates: Partial<Script>)
    const updatedScript = await ScriptService.updateScript(script.id, {
